...
Label(root, text='Convert from:', font=('Georgia', 19, 'bold')).place(x=60, y=60)
...
Label(root, text='Convert to:', font=('Georgia', 19, 'bold')).place(x=330, y=60)
...
TO__currency_names.place(x=300, y=140)
TO__currency_names.current((li_currencies.index("INR - Indian Rupee")))

# Submit Button
submit_btn = Button(root, text='Submit', bg='SlateBlue2', command=lambda: convert_currency(api, converted_currency, FROM__currency_names.get(), TO__currency_names.get(), amnt_from.get()))
